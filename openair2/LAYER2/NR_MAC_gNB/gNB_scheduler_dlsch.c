...
void pf_dl_masked(module_id_t module_id,
           frame_t frame,
           sub_frame_t slot,
           NR_list_t *UE_list,
           int max_num_ue,
  ...
  for (int UE_id = UE_list->head; UE_id >= 0; UE_id = UE_list->next[UE_id]) {
    if (UE_info->Msg4_ACKed[UE_id] != true) continue;
    if (UE_info->is_mt[UE_id] != sched_mt){
      //LOG_I(NR_MAC, "Skipping ue %d\n",UE_id);
      continue; // skip or sched mt depending on the mask
    } else {
      //LOG_I(NR_MAC, "NOT skipping ue %d\n",UE_id);
    }
    NR_UE_sched_ctrl_t *sched_ctrl = &UE_info->UE_sched_ctrl[UE_id];
    if (sched_ctrl->ul_failure==1 && get_softmodem_params()->phy_test==0) continue;
    NR_sched_pdsch_t *sched_pdsch = &sched_ctrl->sched_pdsch;
...
void nr_fr1_dlsch_preprocessor(module_id_t module_id, frame_t frame, sub_frame_t slot)
{
  ...
  if (false){
    LOG_I(NR_MAC,"mt_posHigh %d bwpSize %d",mt_posHigh,bwpSize);
    LOG_I(NR_MAC,"mask %s nrbmt %d nrbtot %d nrbue %d\n",rbgalloc_mt_mask,n_rbg_mt,n_rb_sched, n_rbg_ue);
  }
